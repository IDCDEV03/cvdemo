        ...
        <div class="col-lg-12">
            <div class="breadcrumb-main">
                <h5 class="breadcrumb-title">
                    <a href="{{ $id ? route('admin.sup_list', $id) : route('admin.supply_all') }}" class="text-muted">รายการบริษัทขนส่ง (Supply)</a>
                    <span class="text-muted mx-1">/</span> ลงทะเบียนบริษัทขนส่ง (Supply)
                </h5>
            </div>
        </div>
    </div>
        ...
        <div class="col-xl-8 col-lg-10 col-12">
            ...
            <form action="{{ route('admin.sup_insert') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            ...
                            {{-- บริษัทฯว่าจ้าง --}}
                            <div class="col-12">
                                <label class="form-label fw-600">บริษัทฯว่าจ้าง <span class="text-danger">*</span></label>
                                <select name="company_code" class="form-select @error('company_code') is-invalid @enderror">
                                    <option value="">-- เลือกบริษัท --</option>
                                    @foreach ($companies as $c)
                                        <option value="{{ $c->company_id }}" {{ old('company_code', $id) == $c->company_id ? 'selected' : '' }}>
                                            {{ $c->company_name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <div class="input-group">
                                    <input type="text" name="company_user" id="company_user"
                                        class="form-control @error('company_user') is-invalid @enderror"
                                        value="{{ old('company_user') }}" autocomplete="off"
                                    ...
                                    @error('company_user') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" name="supply_password" id="supply_password"
                                    class="form-control @error('supply_password') is-invalid @enderror"
                                    placeholder="อย่างน้อย 4 ตัวอักษร" autocomplete="off">
                                @error('supply_password') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-3 mb-20">
                    <a href="{{ $id ? route('admin.sup_list', $id) : route('admin.supply_all') }}" class="btn btn-warning px-4">ยกเลิก</a>
...
function generateUsername() {
    ...
    document.getElementById('company_user').value = val;
    checkUsername(val);
}
