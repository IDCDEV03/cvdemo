...
@section('content')
    ...
    <div class="container-fluid">
            ...
            <div class="col-lg-12">
                <div class="breadcrumb-main">
                    ...
                    <div class="d-none d-md-flex align-items-center">
                        <div class="btn-group">
                            <a href="{{ route('vehicles.index') }}" class="btn btn-outline-dark btn-sm" title="ย้อนกลับ">
                                <i class="uil uil-arrow-left"></i> ย้อนกลับ
                            </a>
                            @if ($userRole !== 'company')
                            <a href="{{ route('vehicles.edit', $vehicle->car_id) }}" class="btn btn-primary btn-sm"
                                title="แก้ไข">
                    ...
                    <div class="breadcrumb-action justify-content-center flex-wrap">
                        ...
                        <div class="d-flex d-md-none me-2">
                            <div class="btn-group">
                                <a href="{{ route('vehicles.index') }}" class="btn btn-outline-dark btn-sm"
                                    title="ย้อนกลับ">
                                    <i class="uil uil-arrow-left"></i>
                                </a>
                                @if ($userRole !== 'company')
                                <a href="{{ route('vehicles.edit', $vehicle->car_id) }}"
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item">
                                    <a href="{{ route('vehicles.index') }}">รายการรถ</a>
                                </li>
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-6 d-flex flex-column gap-3">
                ...
                <div class="info-card">
                    ...
                    <div class="info-row">
                        ...
                        <div class="info-value">{{ $vehicle->insurance_company_name ?? ($vehicle->car_insure ?? '-') }}</div>
                    </div>
                </div>
            ...
            <div class="col-md-6 d-flex flex-column gap-3">
                ...
                <div class="info-card">
                    ...
                    <div class="info-row">
                        <div class="info-label">วันที่ตรวจล่าสุด</div>
                        <div class="info-value">
                            @if ($lastInspectDate)
                                <span class="text-success" style="font-size:16px;">
                                    {{ thai_date($lastInspectDate) }}
                                </span>
                            @else
                                <span class="text-muted">ยังไม่มีการตรวจ</span>
                            @endif
                        </div>
                    </div>
                    <div class="info-row">
                        <div class="info-label">วันหมดอายุการตรวจ</div>
                        <div class="info-value">
                            @if ($lastInspectDate)
                                @php
                                    $inspectExpiry = \Carbon\Carbon::parse($lastInspectDate)->addYear();
                                    $diffExpiry = $today->diffInDays($inspectExpiry->copy()->startOfDay(), false);
                                @endphp
                                <span class="{{ $dateClass($inspectExpiry) }}">{{ thai_date($inspectExpiry) }}</span>
                                @if ($diffExpiry < 0)
                                    <span class="text-danger ms-1" style="font-size:14px;">หมดอายุแล้ว</span>
                                @elseif ($diffExpiry <= 30)
                                    <span class="text-warning ms-1" style="font-size:14px;">ใกล้หมด</span>
                                @endif
                            @else
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="d-flex gap-2 align-items-center">
                                   ...
                                   cursor:pointer;">
                                    ...
                                    {{-- Download icon --}}
                                    <a href="{{ route('vehicles.documents.download', [$vehicle->car_id, $activeDocument->id]) }}"
                            ...
                            <div>
                                ...
                                title="คลิกเพื่อดู PDF" @endif>
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <div class="d-flex gap-2 align-items-center">
                                                    {{-- Download --}}
                                                    <a href="{{ route('vehicles.documents.download', [$vehicle->car_id, $doc->id]) }}"
        ...
        {{-- ============================================ --}}
        @if ($supplyHistory->count() > 0)
        <div class="row">
            <div class="col-md-12">
                <div class="card shadow-sm mb-25">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h6 class="mb-0">
                           ประวัติการเปลี่ยน Supply
                        </h6>
                        <span class="dm-tag tag-transparented tag-info fs-18">
                            ทั้งหมด {{ $supplyHistory->count() }} ครั้ง
                        </span>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-bordered mb-0" style="font-size:15px;">
                                ...
                                <tbody>
                                    @foreach ($supplyHistory as $i => $h)
                                    <tr>
                                        <td class="text-center text-muted">{{ $loop->iteration }}</td>
                                        <td>
                                            @if ($h->from_supply_name)
                                               
                                                    {{ $h->from_supply_name }}
                                                
                                            @else
                                                <span class="text-muted fst-italic" style="font-size:13px;">— (ครั้งแรก)</span>
                                            @endif
                                        </td>
                                        <td>
                                           {{ $h->to_supply_name }}
                                        ...
                                        <td>{{ thai_date($h->changed_at) }}</td>
                                        <td class="text-muted">{{ $h->changed_by_name }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
    ...
    <div class="modal fade" id="uploadDocModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('vehicles.documents.upload', $vehicle->car_id) }}" method="POST"
        ...
        $(document).ready(function() {
            ...
            $('.status-option').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: "{{ route('vehicles.ajax.change_status', $vehicle->car_id) }}",
                        method: 'POST',
                        data: {
                            status: newStatus
                        },
...
function confirmDeleteDoc(docId, docName) {
    Swal.fire({
        ...
        const url = '{{ route('vehicles.documents.delete', [$vehicle->car_id, '__DOC_ID__']) }}'
